...
export class AuthController {
  ...
  async loginWith2fa(
    @Body() body: { temp_token: string; code: string },
    @Res({ passthrough: true }) res: Response,
  ) {
    const tokens = await this.authService.loginWith2fa(
      body.temp_token,
      body.code,
    );
    res.cookie('refreshToken', tokens.refresh_token, {
  ...
  async generate2fa(@Req() req: Request) {
    ...
    const qrCodeUrl = await this.twoFactorAuthService.generateQrCodeDataURL(
