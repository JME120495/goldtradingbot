...
export default function AccountsPage() {
  ...
  const fetchAccounts = async () => {
    try {
      ...
      const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL || `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001"}`}` + '/trading-accounts', {
  ...
  const handleAddAccount = async (e: React.FormEvent) => {
    ...
    try {
      const token = Cookies.get('token');
      await axios.post(`${process.env.NEXT_PUBLIC_API_URL || `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001"}`}` + '/trading-accounts', {
        accountNumber,
        broker,
        server
      }, {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const token = Cookies.get('token');
      await axios.delete(`${process.env.NEXT_PUBLIC_API_URL || `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001"}`}/trading-accounts/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      await fetchAccounts();
    } catch (err) {
